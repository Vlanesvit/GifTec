...
function parallaxImage(imageSelector) {
	...
	const easingFactor = 0.1; // Чем меньше число, тем медленнее и плавнее анимация
	...
	images.forEach(image => {
		...
		const update = () => {
			...
			if (rect.top < windowHeight && rect.bottom > 0) {
				...
				targetY = 0 - (30 * scrollProgress); // От 20% до -50%
			}

			currentY = lerp(currentY, targetY, easingFactor);
			image.style.transform = `translateY(${currentY}%)`;
			requestAnimationFrame(update); // Постоянное обновление для плавности
		};
...
parallaxImage('.rs-parallax__bg img');
